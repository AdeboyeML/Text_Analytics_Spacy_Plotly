...
class text_extract:
    ...
    def clean_text(self):
        ...
        chapters = []
        for l in data:
            match = re.search(r'(Chapter.*|Prologue.*|Epilogue.*)\n', l)
            if match:
        ...
        text = []
        for x in data:
            if 'Prologue' or 'Epilogue' in chapters:
                tx = re.sub(r'(CHAPTER.*|PROLOGUE\s|EPILOGUE\s)', 'Chpt', x)
            else:
                tx = re.sub(r'Chapter\s+\d+', r'Chpt', x)
            tx = tx.replace('\n', ' ')
            tx = tx.replace('\t', ' ')
        ...
        #Extract sentence using NLTK sent_tokenize
